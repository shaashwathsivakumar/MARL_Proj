...
if __name__ == '__main__':
    ...
    env, dim_info = simple_tag_v3.parallel_env(render_mode="human", max_cycles= args.episode_length)
    obs, info = env.reset()
    ...
    for episode in range(args.episode_num):
        ...
        while env.agents:  # interact with the env for an episode
            actions = {}
            for agent in agents:
                action = agents[agent].get_action(observations[agent], model_out = False)
